...
//
//    You do not need to be the next Gauss to figure that all numbers 
//    with 1 or 2 digits are either increasing or decreasing: 
//    00, 01, 02, ..., 98, 99 are all belonging to one of this categories 
//    (if not both, like 22 or 55): 101 is indeed the first number which 
//    does NOT fall into either of the categories. Same goes for all the 
...
//    all the increasing or decreasing numbers below 10 raised to the 
//    xth power (x will always be >= 0).
//
...
//    with a brute force approach, some tests with larger numbers may take 
//    more than the total computing power currently on Earth to be finished 
//    in the short allotted time.
//
//    To make it even clearer, the increasing or decreasing numbers between 
//    in the range 101-200 are: 
//    [110, 111, 112, 113, 114, 115, 116, 117, 118, 119, 122, 123, 124, 125, 
//    126, 127, 128, 129, 133, 134, 135, 136, 137, 138, 139, 144, 145, 146,
//    147, 148, 149, 155, 156, 157, 158, 159, 166, 167, 168, 169, 177, 178, 
//    179, 188, 189, 199, 200], that is 47 of them. 
//    In the following range, 201-300, there are 41 of them and so on, 
//    getting rarer and rarer.
//
//    Trivia: just for the sake of your own curiosity, a number which is 
//    neither decreasing of increasing is called a bouncy number, like, say, 
//    3848 or 37294; also, usually 0 is not considered being increasing, 
//    decreasing or bouncy, but it will be for the purpose of this kata

    
    public static BigInteger totalIncDec(int x){
        BigInteger result = BigInteger.ZERO;
        BigInteger repeats = BigInteger.valueOf(10);
        ...
        if (x == 0)
            ...
            result = result.add(BigInteger.valueOf(100));
        else {
            int y = x-1;
            result = result.add(decreasing(y).add(increasing(y)).subtract(repeats));
            if (x >= 1)
                result = result.add(totalIncDec(x-1));
        }
        return result;
    }
